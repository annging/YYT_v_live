...
(function(window, undefined) {
	...
	var parseMessage = function(messageText, pushstream) {
		var msg = messageText;
		var channel = "";
		...
		if (typeof msg == "object") {
			message = {
				id : msg[pushstream.jsonIdKey],
				channel : msg[pushstream.jsonChannelKey],
				data : unescapeText(msg[pushstream.jsonDataKey]),
				tag : msg[pushstream.jsonTagKey],
				time : msg[pushstream.jsonTimeKey],
				eventid : msg[pushstream.jsonEventIdKey] || ""
			};
		} else {
			message = {data : msg, id : 1, channel : channel};
		}
		return message;
	};
	...
	var onerrorCallback = function(event) {
		...
		this.pushstream._onerror({type : ((event && (event.type === "load")) || (this.pushstream.readyState === PushStream.CONNECTING)) ?
				"load" : "timeout"});
	}
	...
	StreamWrapper.prototype = {
		...
		process : function(id, channel, data, eventid, time) {
			this.pingtimer = clearTimer(this.pingtimer);
			// Log4js.info("[Stream] message received", arguments);
			this.pushstream._onmessage(unescapeText(data), id, channel, eventid, true, time);
			this.setPingTimer();
		},
	...
	var PushStream = function(settings) {
		...
		this.reconnecttimeout = settings.reconnecttimeout || 1000;
	...
	PushStream.prototype = {
		...
		_onmessage : function(data, id, channel, eventid, isLastMessageFromBatch, time) {
			...
			if (id == -2) {
				...
				if (this.onmessage) { this.onmessage(data, id, channel, eventid, isLastMessageFromBatch, time); }
			}
		},
