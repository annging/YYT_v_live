define(function(require, exports, module) {
	...
	ScrollBar.prototype.update = function(direction) {
		...
		if (direction != 'undefind') {
			if (direction == 'top') {
				...
				this.position();
			} else if (typeof direction == 'number') { //接受数值
				this.scaleScroll = direction / this.content.height();
				this.t = this.bar.height() * this.scaleScroll;
	...
	// 上下
	ScrollBar.prototype.position = function(update_top) {
		...
		if (this.options.axis == 'y') {
			...
			if (!update_top) {
				...
				if (this.relative == 0) {
					...
					if (update_top > (this.content.height() - this.contentParent.height())) {
						update_top = this.content.height() - this.contentParent.height();
					} else {
						update_top = update_top;
					}
					// 记住上次位置
					this.relative = update_top;
