define(function() {
	...
	function onInput(element, callback) {
		...
		if (window.addEventListener && !window.atob) {
			$element.on('keydown', function(e) { //fixed IE9 backspace delete ctrl-z event
				var key = e.keyCode;
			...
			$element.on('contextmenu', function() { //fixed IE9 contextmenu -> undo cut paste delete event
				callFn();
			});
		}
