...
(function() {
	...
	var tmp = window.location.pathname,
			URL = "/js/ueditor/";//这里你可以配置成ueditor目录在您网站的相对路径或者绝对路径（指以http开头的绝对路径）
	UEDITOR_CONFIG = {
		...
		maxImageSideLength : 800, //上传图片最大允许的边长，超过会自动等比缩放,不缩放就设置一个比较大的值
		relativePath : true, //是否开启相对路径。开启状态下所有本地图片的路径都将以相对路径形式进行保存.强烈建议开启！
		uploadImgParams : '{"belongId":"456","editorUploadType":"myBlog"}', //上传图片时的参数
		UEDITOR_HOME_URL : URL, //为editor添加一个全局路径
		//工具栏上的所有的功能按钮和下拉框，可以在new编辑器的实例时选择自己需要的从新定义
		toolbars : [
			['FontFamily', 'FontSize', 'Bold', 'Italic', 'Underline', 'ForeColor', 'BackColor', 'Spechars',
				'JustifyLeft', 'JustifyCenter', 'JustifyRight', 'JustifyJustify', 'InsertTable',
				'Emotion', 'InsertImage', 'InsertVideo', 'Link',
				'SelectAll', 'Preview']
		],
		...
		iframeUrlMap : {
			...
			'insertimage' : '~/dialogs/image/image522.html',
			...
			'insertvideo' : '~/dialogs/video/video5_16.html',
		...
		listMap : {
			//字体
			'fontfamily' : ['宋体', '黑体', '楷体', '微软雅黑', 'Times New Roman', 'Arial', 'Verdana'],
			//字号
		...
		//选中的td上的样式
				'.selectTdClass{background-color:#3399FF !important}' +
					...
					//锚点的样式,注意这里背景图的路径
						'.anchorclass{background: url("' + URL +
					...
					//设置四周的留边
						'.view{padding:0;word-wrap:break-word;word-break:break-all;cursor:text;height:100%;background: url("' + URL +
						'yinyuetai-logo.png") no-repeat fixed right bottom transparent}\n' +
